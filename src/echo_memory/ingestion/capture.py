...
def notice(conn, path: str, project: str, digest: str) -> dict:
    ...
    row = conn.execute(
        """
        INSERT INTO public.pending_ingest (path, project, digest)
        VALUES (%s, %s, %s)
...
def notice_file(conn, path: str | Path, project: str) -> dict:
    p = Path(path)
    return notice(conn, str(p), project, digest_of(p.read_text()))


def pending(conn, project: str | None = None) -> list[dict]:
    sql = """SELECT path, project, digest, noticed_at FROM public.pending_ingest
    ...
    return [
        {"path": r[0], "project": r[1], "digest": r[2], "noticed_at": r[3]}
